...
import { Users, UserPlus, LogOut, KeyRound, Copy, UserMinus } from 'lucide-react'
        ...
        {!myTeam ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
              ...
              <div className="space-y-3">
                ...
                <button onClick={createTeam} disabled={loading} className="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded horror-glow">{loading ? 'Creating...' : 'Create'}</button>
              </div>
            </div>
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
              ...
              <div className="space-y-3">
                ...
                <button onClick={joinTeam} disabled={loading} className="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded horror-glow">{loading ? 'Joining...' : 'Join'}</button>
              </div>
            </div>
          </div>
        ) : (
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            <div className="flex items-center justify-between">
              ...
              <button onClick={leaveTeam} disabled={loading} className="px-3 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded"><LogOut className="h-4 w-4 inline mr-1"/> Leave</button>
            </div>
                ...
                <div key={m.user.id} className="flex items-center justify-between text-sm text-gray-200">
                  ...
                  {user?.id === myTeam.leaderId && m.user.id !== user.id && (
                    <button
                      ...
                      className="px-2 py-1 text-xs bg-red-600 hover:bg-red-700 rounded flex items-center gap-1"
                    >
